...
class Actor(nn.Module):
    def __init__(self, args, agent_id):
        ...
        self.fc1 = nn.Linear(args.obs_dim_n[agent_id], args.hidden_dim)  # 输入: 每个agent的obs
        self.fc2 = nn.Linear(args.hidden_dim, args.hidden_dim)  # hidden 1 2
        self.fc3 = nn.Linear(args.hidden_dim, args.action_dim_n[agent_id])  # 输出: 每个agent的action
        if args.use_orthogonal_init:  # 正交初始化
...
class Critic_MADDPG(nn.Module):
    def __init__(self, args):
        ...
        self.fc1 = nn.Linear(sum(args.obs_dim_n) + sum(args.action_dim_n), args.hidden_dim)  # 输入: 所有agent的obs、action
        self.fc2 = nn.Linear(args.hidden_dim, args.hidden_dim)  # hidden 1 2
        self.fc3 = nn.Linear(args.hidden_dim, 1)  # 输出: 动作价值
    ...
    def forward(self, s, a):
        s = torch.cat(s, dim=1)  # total_obs
        a = torch.cat(a, dim=1)  # total_action
        s_a = torch.cat([s, a], dim=1)  # total_obs + total_action
