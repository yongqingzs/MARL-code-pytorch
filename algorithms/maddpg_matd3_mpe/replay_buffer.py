...
class ReplayBuffer(object):
    def __init__(self, args):
        ...
        for agent_id in range(self.N):
            # r、done为什么是1: 因为每个agent都只会获得一个reward和done
    ...
    def store_transition(self, obs_n, a_n, r_n, obs_next_n, done_n):
        """
        current_size到达buffer_size后，就不再增长，而maddpg中current_size到达
        batch_size后，就开始更新策略
        """
        for agent_id in range(self.N):
        ...
        self.current_size = min(self.current_size + 1, self.buffer_size)  # current_size + 1，并且取其与buffer_size的小值
